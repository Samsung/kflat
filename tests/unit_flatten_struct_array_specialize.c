...
FUNCTION_DEFINE_FLATTEN_STRUCT_ITER_SELF_CONTAINED_SPECIALIZE(
    ints, spec_array, sizeof(struct spec_array),
    AGGREGATE_FLATTEN_TYPE_ARRAY_SELF_CONTAINED(
        int, integers, offsetof(struct spec_array, integers), 10
    );
);

FUNCTION_DEFINE_FLATTEN_STRUCT_ITER_SELF_CONTAINED_SPECIALIZE(
    strs, spec_array, sizeof(struct spec_array),
    AGGREGATE_FLATTEN_STRING_SELF_CONTAINED(string, offsetof(struct spec_array, string));
);

FUNCTION_DEFINE_FLATTEN_STRUCT_ITER_SELF_CONTAINED_SPECIALIZE(
    magic, spec_array, sizeof(struct spec_array)
);
