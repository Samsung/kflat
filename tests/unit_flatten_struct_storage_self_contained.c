...
FUNCTION_DEFINE_FLATTEN_STRUCT(MMS,
    AGGREGATE_FLATTEN_STRING(s);
    AGGREGATE_FLATTEN_STRUCT_ARRAY_STORAGE_SELF_CONTAINED(BBS, arrB, offsetof(struct MMS,arrB), 4);
    AGGREGATE_FLATTEN_UNION_ARRAY_STORAGE_SELF_CONTAINED(KS, arrK0, offsetof(struct MMS,arrK0), 2);
    AGGREGATE_FLATTEN_TYPE_ARRAY(long, Lx, 0);
    AGGREGATE_FLATTEN_UNION_ARRAY_STORAGE_CUSTOM_INFO_SELF_CONTAINED(KS, arrK, offsetof(struct MMS,arrK), 2, ATTR(has_s));
);
...
FUNCTION_DEFINE_FLATTEN_UNION(SKS,
    AGGREGATE_FLATTEN_STRUCT_TYPE_STORAGE_SELF_CONTAINED(string_s_t,us,offsetof(union SKS,us));
);

FUNCTION_DEFINE_FLATTEN_STRUCT(SAS,
    AGGREGATE_FLATTEN_UNION_STORAGE_SELF_CONTAINED(SKS,u,offsetof(struct SAS,u));
    AGGREGATE_FLATTEN_UNION_STORAGE_CUSTOM_INFO_SELF_CONTAINED(SKS,w,offsetof(struct SAS,w),&ATTR(l));
    AGGREGATE_FLATTEN_STRUCT_STORAGE_SELF_CONTAINED(str_s,ss,offsetof(struct SAS,ss));
    AGGREGATE_FLATTEN_STRUCT_TYPE_ARRAY_STORAGE_SELF_CONTAINED(string_s_t,sarr,offsetof(struct SAS,sarr),4);
);
...
KFLAT_REGISTER_TEST("[UNIT] flatten_struct_storage_self_contained", kflat_flatten_struct_storage_self_contained_unit_test, kflat_flatten_struct_storage_self_contained_unit_validate);
