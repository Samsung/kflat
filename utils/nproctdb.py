...
static void handler_{2}(struct kflat* kflat, struct probe_regs* regs) {{
...
def main():
	...
	parser.add_argument("struct", help="struct type for which kflat recipes will be generated")
	...
	Ri = [x.id for x in RG.ftdb.types if x.classname=="record" and x.str == args.struct and not RG.DI.isTypeConst(x)]
	if len(Ri) == 0:
		print(f"EE- Failed to locate structure {args.struct}")
		exit(1)
	for R in Ri:
		T = RG.ftdb.types[R]
		deps.add((T.id,T.str))
	...
	for x in set(RG.structs_done_match) - set(RG.structs_missing):
		...
		if name == args.struct:
			...
			recipe_handlers_stream.write(RecipeGenerator.template_output_recipe_handler.format(name, 1, args.func))
	...
	with open(os.path.join(args.output,"Kbuild"),"w") as f:
		f.write(RecipeGenerator.template_kbuild_recipes%("kflat"," \\\n".join(["    %s"%(x) for x in objs]), "kflat"))
